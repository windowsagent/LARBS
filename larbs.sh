...
# License: GNU GPLv3
# Repurposed by Knox, for use by Knox
...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/windowsagent/voidrice"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/windowsagent/LARBS/master/progs.csv"
...
welcomemsg() { \
	dialog --title "Greetings!" --msgbox "Welcome to the post installation part of the Modified-Arch-Install-script (patent pending).\\n After clicking ok, you'll be prompted to configure a username, and the configuration files will automatically install!" 10 60
...
preinstallmsg() { \
	dialog --title "Let's get this party started OwO" --yes-label "Yes please uwu" --no-label "Noup!" --yesno "The rest of the installation will now be totally automated, so you can sit back relax, and watch Zootopia..\\n\\nIt will take some time, but when done, you can relax even more with your compwete system.\\n\\nNow just press <Let's get this party started OwO> and the system will begin installation!" 13 60 || { clear; exit 1; }
	}
...
dialog --title "LARBS Installation" --infobox "Installing rust toolchain..." 5 70
yes | sudo -u "$name" $aurhelper -S rustup >/dev/null 2>&1
...
# Setup greeterd conf
curl -K "https://raw.githubusercontent.com/windowsagent/LARBS/master/greetd-config.toml" > /etc/greetd/config.toml
